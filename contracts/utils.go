...
import (
	...
	randomizeContract "github.com/CratD2C-SmartChain/cratd2cchain/contracts/randomize/contract"
...
func CreateTransactionSign(chainConfig *params.ChainConfig, pool *core.TxPool, manager *accounts.Manager, block *types.Block, chainDb ethdb.Database, eb common.Address) error {
	...
	if chainConfig.CratD2C != nil {
		...
		checkNumber := blockNumber % chainConfig.CratD2C.Epoch
		...
		if !exist && checkNumber > 0 && common.EpocBlockSecret <= checkNumber && common.EpocBlockOpening > checkNumber {
			...
			tx, err := BuildTxSecretRandomize(nonce+1, common.HexToAddress(common.RandomizeSMC), chainConfig.CratD2C.Epoch, randomizeKeyValue)
			if err != nil {
...
func GetRewardForCheckpoint(c *CratD2C.CratD2C, chain consensus.ChainReader, header *types.Header, rCheckpoint uint64, totalSigner *uint64) (map[common.Address]*RewardLog, error) {
