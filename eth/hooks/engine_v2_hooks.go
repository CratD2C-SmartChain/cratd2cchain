...
func AttachConsensusV2Hooks(adaptor *CratD2C.CratD2C, bc *core.BlockChain, chainConfig *params.ChainConfig) {
	// Hook scans for bad masternodes and decide to penalty them
	// Subnet penalty is triggered at gap block, and stopped at previous gap block
	adaptor.EngineV2.HookPenalty = func(chain consensus.ChainReader, number *big.Int, currentHash common.Hash, candidates []common.Address, config *params.CratD2CConfig) ([]common.Address, error) {
		start := time.Now()
	...
	adaptor.EngineV2.HookReward = func(chain consensus.ChainReader, stateBlock *state.StateDB, parentState *state.StateDB, header *types.Header) (map[string]interface{}, error) {
		...
		foundationWalletAddr := chain.Config().CratD2C.FoudationWalletAddr
		...
		if number == chain.Config().CratD2C.V2.SwitchBlock.Uint64()+1 {
			return rewards, nil
		}
		start := time.Now()
		// Get reward inflation.
		chainReward := new(big.Int).Mul(new(big.Int).SetUint64(chain.Config().CratD2C.Reward), new(big.Int).SetUint64(params.Ether))
...
func GetSigningTxCount(c *CratD2C.CratD2C, chain consensus.ChainReader, header *types.Header, totalSigner *uint64) (map[common.Address]*contracts.RewardLog, error) {
	...
	for i := number - 1; ; i-- {
		...
		if isEpochSwitch && i != chain.Config().CratD2C.V2.SwitchBlock.Uint64()+1 {
			epochCount += 1
