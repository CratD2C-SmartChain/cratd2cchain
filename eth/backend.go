...
import (
	...
	"github.com/CratD2C-SmartChain/cratd2cchain/eth/filters"
	...
	"github.com/CratD2C-SmartChain/cratd2cchain/internal/ethapi"
	"github.com/CratD2C-SmartChain/cratd2cchain/log"
	"github.com/CratD2C-SmartChain/cratd2cchain/miner"
...
func New(ctx *node.ServiceContext, config *Config, XDCXServ *XDCx.XDCX, lendingServ *XDCxlending.Lending) (*Ethereum, error) {
	...
	if eth.chainConfig.CratD2C != nil {
		c := eth.engine.(*CratD2C.CratD2C)
	...
	if eth.chainConfig.CratD2C != nil {
		eth.ApiBackend = &EthApiBackend{eth, nil, eth.engine.(*CratD2C.CratD2C)}
	} else {
...
func CreateConsensusEngine(ctx *node.ServiceContext, config *ethash.Config, chainConfig *params.ChainConfig, db ethdb.Database) consensus.Engine {
	...
	if chainConfig.CratD2C != nil {
		return CratD2C.New(chainConfig, db)
	}
...
func (s *Ethereum) ValidateMasternode() (bool, error) {
	...
	if s.chainConfig.CratD2C != nil {
		//check if miner's wallet is in set of validators
		c := s.engine.(*CratD2C.CratD2C)
		...
		return false, fmt.Errorf("Only verify masternode permission in CratD2C protocol")
	}
	return true, nil
}
...
func (s *Ethereum) StartStaking(local bool) error {
	...
	if XDPoS, ok := s.engine.(*CratD2C.CratD2C); ok {
