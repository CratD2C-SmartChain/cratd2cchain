...
func NewLDBEngine(cfg *Config) *DCxDAO.BatchDatabase {
	datadir := cfg.DataDir
	batchDB := DCxDAO.NewBatchDatabaseWithEncode(datadir, 0)
	return batchDB
}

func NewMongoDBEngine(cfg *Config) *DCxDAO.MongoDatabase {
	mongoDB, err := DCxDAO.NewMongoDatabase(nil, cfg.DBName, cfg.ConnectionUrl, cfg.ReplicaSetName, 0)
...
func (XDCx *XDCX) GetLevelDB() DCxDAO.XDCXDAO {
	return XDCx.db
}

func (XDCx *XDCX) GetMongoDB() DCxDAO.XDCXDAO {
	return XDCx.mongodb
}
...
func (XDCx *XDCX) SyncDataToSDKNode(takerOrderInTx *tradingstate.OrderItem, txHash common.Hash, txMatchTime time.Time, statedb *state.StateDB, trades []map[string]string, rejectedOrders []*tradingstate.OrderItem, dirtyOrderCount *uint64) error {
	...
	*dirtyOrderCount++
	...
	for _, trade := range trades {
		...
		//updatedTakerOrder = DCx.updateMatchedOrder(updatedTakerOrder, filledAmount, txMatchTime, txHash)
...
func (XDCx *XDCX) GetTradingState(block *types.Block, author common.Address) (*tradingstate.TradingStateDB, error) {
	...
	if XDCx.StateCache == nil {
		return nil, errors.New("Not initialized DCx")
	}
...
func (XDCx *XDCX) RollbackReorgTxMatch(txhash common.Hash) error {
	...
	if items != nil {
		for _, order := range items.([]*tradingstate.OrderItem) {
			c, ok := XDCx.orderCache.Get(txhash)
			log.Debug("DCx reorg: rollback order", "txhash", txhash.Hex(), "order", tradingstate.ToJSON(order), "orderHistoryItem", c)
			if !ok {
				log.Debug("DCx reorg: remove order due to no orderCache", "order", tradingstate.ToJSON(order))
			...
			if (orderHistoryItem == tradingstate.OrderHistoryItem{}) {
				log.Debug("DCx reorg: remove order due to empty orderHistory", "order", tradingstate.ToJSON(order))
				if err := db.DeleteObject(order.Hash, &tradingstate.OrderItem{}); err != nil {
					log.Crit("SDKNode: failed to remove reorg order", "err", err.Error(), "order", tradingstate.ToJSON(order))
				}
				continue
			...
			log.Debug("DCx reorg: update order to the last orderHistoryItem", "order", tradingstate.ToJSON(order), "orderHistoryItem", orderHistoryItem)
			if err := db.PutObject(order.Hash, order); err != nil {
				log.Crit("SDKNode: failed to update reorg order", "err", err.Error(), "order", tradingstate.ToJSON(order))
			}
		}
	}
	log.Debug("DCx reorg: DeleteTradeByTxHash", "txhash", txhash.Hex())
