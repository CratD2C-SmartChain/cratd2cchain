...
func (s *Service) loop() {
	...
	if s.eth != nil {
		...
		// txpool = s.eth.TxPool()
		// engine = s.eth.Engine().(*CratD2C.CratD2C)
	} else {
...
func (s *Service) reportLatency(conn *connWrapper) error {
	...
	case <-time.After(5 * time.Second):
		// Ping timeout, abort
		// return errors.New("ping timed out")			//temporary fix miners-stats DC issue.
	}
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	if (s.engine.(*CratD2C.CratD2C).EngineV2 != nil) && (s.engine.(*CratD2C.CratD2C).EngineV2.GetLatestCommittedBlockInfo() != nil) {
		latestCommittedBlockInfo := s.engine.(*CratD2C.CratD2C).EngineV2.GetLatestCommittedBlockInfo()
