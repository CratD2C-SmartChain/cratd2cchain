...
func (s *PublicBlockChainAPI) GetPreviousCheckpointFromEpoch(ctx context.Context, epochNum rpc.EpochNumber) (rpc.BlockNumber, rpc.EpochNumber) {
	...
	if engine, ok := s.b.GetEngine().(*CratD2C.CratD2C); ok {
		...
		panic("[GetPreviousCheckpointFromEpoch] Error while trying to get CratD2C consensus engine")
	}
}
...
func (s *PublicBlockChainAPI) findNearestSignedBlock(ctx context.Context, b *types.Block) *types.Block {
	...
	// Get block epoc latest
	checkpointNumber, _, err := s.b.GetEngine().(*CratD2C.CratD2C).GetCurrentEpochSwitchBlock(s.chainReader, big.NewInt(int64(signedBlockNumber)))
	if err != nil {
...
func (s *PublicBlockChainAPI) findFinalityOfBlock(ctx context.Context, b *types.Block, masternodes []common.Address) (uint, error) {
	engine, _ := s.b.GetEngine().(*CratD2C.CratD2C)
...
*/
func (s *PublicBlockChainAPI) getSigners(ctx context.Context, block *types.Block, engine *CratD2C.CratD2C) ([]common.Address, error) {
	var err error
...
func (s *PublicBlockChainAPI) rpcOutputBlockSigners(b *types.Block, ctx context.Context, masternodes []common.Address) ([]common.Address, error) {
	...
	engine, ok := s.b.GetEngine().(*CratD2C.CratD2C)
	if !ok {
		log.Error("Undefined CratD2C consensus engine")
...
func GetSignersFromBlocks(b Backend, blockNumber uint64, blockHash common.Hash, masternodes []common.Address) ([]common.Address, error) {
	...
	if engine, ok := b.GetEngine().(*CratD2C.CratD2C); ok {
...
func (s *PublicBlockChainAPI) GetStakerROI() float64 {
	...
	lastCheckpointNumber := blockNumber - (blockNumber % s.b.ChainConfig().CratD2C.Epoch) - s.b.ChainConfig().CratD2C.Epoch // calculate for 2 epochs ago
	...
	masternodeReward := new(big.Int).Mul(new(big.Int).SetUint64(s.b.ChainConfig().CratD2C.Reward), new(big.Int).SetUint64(params.Ether))
...
func (s *PublicBlockChainAPI) GetStakerROIMasternode(masternode common.Address) float64 {
	...
	blockNumber := s.b.CurrentBlock().Number().Uint64()
	lastCheckpointNumber := blockNumber - blockNumber%s.b.ChainConfig().CratD2C.Epoch
	totalCap := new(big.Int).SetUint64(0)
