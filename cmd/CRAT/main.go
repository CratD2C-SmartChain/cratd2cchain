...
const (
	clientIdentifier = "CRAT" // Client identifier to advertise over the network
)
...
func init() {
	// Initialize the CLI app and start CRAT
	app.Action = XDC
...
func startNode(ctx *cli.Context, stack *node.Node, cfg XDCConfig) {
	...
	if engine, ok := ethereum.Engine().(*CratD2C.CratD2C); ok {
		go func() {
			started := false
			ok := false
