...
const (
	ipcAPIs  = "XDCx:1.0 XDCxlending:1.0 CratD2C:1.0 admin:1.0 debug:1.0 eth:1.0 miner:1.0 net:1.0 personal:1.0 rpc:1.0 txpool:1.0 web3:1.0"
...
func TestConsoleWelcome(t *testing.T) {
	...
	// Start a CRAT console, make sure it's cleaned up and terminate the console
	XDC := runXDC(t,
	...
	XDC.Expect(`
Welcome to the CRAT JavaScript console!

instance: CRAT/v{{XDCver}}/{{goos}}-{{goarch}}/{{gover}}
coinbase: {{.Etherbase}}
at block: 0 ({{niltime}})
 datadir: {{.Datadir}}
 modules: {{apis}}
...
func testAttachWelcome(t *testing.T, XDC *testXDC, endpoint, apis string) {
	// Attach to a running CRAT note and terminate immediately
