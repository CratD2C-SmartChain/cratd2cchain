...
import (
	...
	validatorContract "github.com/CratD2C-SmartChain/cratd2cchain/contracts/validator"
...
func (w *wizard) makeGenesisFile() {
	...
	genesis.Config.CratD2C = &params.CratD2CConfig{
		Period: 15,
		Epoch:  30000,
		Reward: 0,
		V2: &params.V2{
	...
	w.network = input.Name
	genesis.Config.CratD2C.NetworkName = input.Name
	genesis.Config.CratD2C.Denom = input.Denom
	fmt.Println()
	fmt.Println("How many seconds should blocks take? (default = 2)")
	// genesis.Config.CratD2C.Period = uint64(w.readDefaultInt(2))
	fmt.Println(input.Period)
	genesis.Config.CratD2C.Period = input.Period
	genesis.Config.CratD2C.V2.CurrentConfig.MinePeriod = int(genesis.Config.CratD2C.Period)

	fmt.Println()
	fmt.Println("How many Ethers should be rewarded to masternode? (default = 10)")
	// genesis.Config.CratD2C.Reward = uint64(w.readDefaultInt(10))
	fmt.Println(input.Reward)
	genesis.Config.CratD2C.Reward = input.Reward

	fmt.Println()
	fmt.Println("How long is the v2 timeout period? (default = 10)")
	// genesis.Config.CratD2C.V2.CurrentConfig.TimeoutPeriod = w.readDefaultInt(10)
	fmt.Println(input.TimeoutPeriod)
	genesis.Config.CratD2C.V2.CurrentConfig.TimeoutPeriod = input.TimeoutPeriod

	fmt.Println()
	fmt.Println("How many v2 timeout reach to send Synchronize message? (default = 3)")
	// genesis.Config.CratD2C.V2.CurrentConfig.TimeoutSyncThreshold = w.readDefaultInt(3)
	fmt.Println(input.TimeoutSyncThreshold)
	genesis.Config.CratD2C.V2.CurrentConfig.TimeoutSyncThreshold = input.TimeoutSyncThreshold
	...
	// genesis.Config.CratD2C.V2.CurrentConfig.CertThreshold = w.readDefaultInt(common.MaxMasternodes/3*2 + 1)
	fmt.Println(input.CertThreshold)
	minCandidateThreshold := input.CertThreshold
	genesis.Config.CratD2C.V2.CurrentConfig.CertThreshold = minCandidateThreshold

	genesis.Config.CratD2C.V2.AllConfigs[0] = genesis.Config.CratD2C.V2.CurrentConfig
	...
	genesis.Config.CratD2C.Epoch = epochNumber
	genesis.Config.CratD2C.RewardCheckpoint = epochNumber

	fmt.Println()
	fmt.Println("How many blocks before checkpoint need to prepare new set of masternodes? (default = 450)")
	// genesis.Config.CratD2C.Gap = uint64(w.readDefaultInt(450))
	fmt.Println(input.Gap)
	genesis.Config.CratD2C.Gap = input.Gap

	fmt.Println()
	fmt.Println("What is foundation wallet address? (default = xdc0000000000000000000000000000000000000068)")
	// genesis.Config.CratD2C.FoudationWalletAddr = w.readDefaultAddress(common.HexToAddress(common.FoudationAddr))
	fmt.Println(input.Grandmasters[0])
	genesis.Config.CratD2C.FoudationWalletAddr = input.Grandmasters[0]
