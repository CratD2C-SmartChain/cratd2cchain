...
import (
	...
	"github.com/CratD2C-SmartChain/cratd2cchain/consensus/misc"
...
func (self *worker) update() {
	...
	minePeriod := 2
	MinePeriodCh := self.engine.(*CratD2C.CratD2C).MinePeriodCh
	...
	for {
		...
		case ev := <-self.txCh:
			...
			if atomic.LoadInt32(&self.mining) == 0 {
				...
				if self.config.CratD2C != nil && self.config.CratD2C.Period == 0 {
					self.commitNewWork()
				}
			}
...
func (self *worker) wait() {
	for {
		...
		for result := range self.recv {
			...
			block := result.Block
			if self.config.CratD2C != nil && block.NumberU64() >= self.config.CratD2C.Epoch && len(block.Validator()) == 0 {
			...
			if work.config.CratD2C != nil {
				// epoch block
				isEpochSwitchBlock, _, err := self.engine.(*CratD2C.CratD2C).IsEpochSwitch(block.Header())
				if err != nil {
...
func (self *worker) commitNewWork() {
	...
	if self.config.CratD2C != nil {
		isEpochSwitchBlock, _, err := self.engine.(*CratD2C.CratD2C).IsEpochSwitch(header)
		if err != nil {
	...
	if atomic.LoadInt32(&self.mining) == 1 {
		...
		if self.config.CratD2C != nil && self.chain.Config().IsTIPXDCX(header.Number) {
			XDCX := self.eth.GetXDCX()
			XDCXLending := self.eth.GetXDCXLending()
			if XDCX != nil && header.Number.Uint64() > self.config.CratD2C.Epoch {
				isEpochSwitchBlock, epochNumber, err := self.engine.(*CratD2C.CratD2C).IsEpochSwitch(header)
				if err != nil {
				...
				if isEpochSwitchBlock {
					...
					//https://github.com/CratD2C-SmartChain/cratd2cchain-v1/pull/416
					...
					if header.Number.Uint64()%self.config.CratD2C.Epoch == common.LiquidateLendingTradeBlock {
...
func (env *Work) commitTransactions(mux *event.TypeMux, balanceFee map[common.Address]*big.Int, txs *types.TransactionsByPriceAndNonce, specialTxs types.Transactions, bc *core.BlockChain, coinbase common.Address) {
	...
	for _, tx := range specialTxs {
		...
		if tx.To().Hex() == common.BlockSigners {
			...
			if blkNumber >= env.header.Number.Uint64() || blkNumber <= env.header.Number.Uint64()-env.config.CratD2C.Epoch*2 {
