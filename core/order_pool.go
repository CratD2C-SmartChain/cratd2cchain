...
func (pool *OrderPool) reset(oldHead, newblock *types.Block) {
	if !pool.chainconfig.IsTIPXDCX(pool.chain.CurrentBlock().Number()) || pool.chain.Config().CratD2C == nil || pool.chain.CurrentBlock().NumberU64() <= pool.chain.Config().CratD2C.Epoch {
		return
	}
...
func (pool *OrderPool) validateOrder(tx *types.OrderTransaction) error {
	...
	if !tx.IsCancelledOrder() {
		...
		if orderType == OrderTypeLimit {
			XDPoSEngine, ok := pool.chain.Engine().(*CratD2C.CratD2C)
			if !ok {
				return ErrNotXDPoS
			}
