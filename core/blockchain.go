...
import (
	...
	"github.com/CratD2C-SmartChain/cratd2cchain/common/mclock"
...
func (bc *BlockChain) loadLastState() error {
	...
	if err != nil {
		...
		engine, ok := bc.Engine().(*CratD2C.CratD2C)
		if ok {
			tradingService := engine.GetXDCXService()
			lendingService := engine.GetLendingService()
			if bc.Config().IsTIPXDCX(currentBlock.Number()) && bc.chainConfig.CratD2C != nil && currentBlock.NumberU64() > bc.chainConfig.CratD2C.Epoch && tradingService != nil && lendingService != nil {
	...
	if engine, ok := bc.Engine().(*CratD2C.CratD2C); ok {
...
func (bc *BlockChain) OrderStateAt(block *types.Block) (*tradingstate.TradingStateDB, error) {
	...
	if ok {
		...
		if bc.Config().IsTIPXDCX(block.Number()) && bc.chainConfig.CratD2C != nil && block.NumberU64() > bc.chainConfig.CratD2C.Epoch && XDCXService != nil {
...
func (bc *BlockChain) LendingStateAt(block *types.Block) (*lendingstate.LendingStateDB, error) {
	...
	if ok {
		lendingService := engine.GetLendingService()
		if bc.Config().IsTIPXDCX(block.Number()) && bc.chainConfig.CratD2C != nil && block.NumberU64() > bc.chainConfig.CratD2C.Epoch && lendingService != nil {
			author, _ := bc.Engine().Author(block.Header())
		...
		if (common.Rewound == uint64(0)) || ((*head).Number().Uint64() < common.Rewound) {
			if _, err := state.New((*head).Root(), bc.stateCache); err == nil {
				...
				if ok {
					tradingService := engine.GetXDCXService()
					lendingService := engine.GetLendingService()
					if bc.Config().IsTIPXDCX((*head).Number()) && bc.chainConfig.CratD2C != nil && (*head).NumberU64() > bc.chainConfig.CratD2C.Epoch && tradingService != nil && lendingService != nil {
...
func (bc *BlockChain) insert(block *types.Block) {
	...
	if bc.chainConfig.CratD2C != nil && !bc.chainConfig.IsTIPSigning(block.Number()) {
		engine, ok := bc.Engine().(*CratD2C.CratD2C)
		if ok {
...
func (bc *BlockChain) HasFullState(block *types.Block) bool {
	...
	engine, _ := bc.Engine().(*CratD2C.CratD2C)
	if bc.Config().IsTIPXDCX(block.Number()) && bc.chainConfig.CratD2C != nil && engine != nil && block.NumberU64() > bc.chainConfig.CratD2C.Epoch {
		tradingService := engine.GetXDCXService()
		lendingService := engine.GetLendingService()
		author, _ := bc.Engine().Author(block.Header())
...
func (bc *BlockChain) SaveData() {
	...
	if !bc.cacheConfig.Disabled {
		...
		if bc.Config().IsTIPXDCX(bc.CurrentBlock().Number()) && bc.chainConfig.CratD2C != nil && bc.CurrentBlock().NumberU64() > bc.chainConfig.CratD2C.Epoch && engine != nil {
			tradingService = engine.GetXDCXService()
		...
		for _, offset := range []uint64{0, 1, triesInMemory - 1} {
			if number := bc.CurrentBlock().NumberU64(); number > offset {
				...
				if bc.Config().IsTIPXDCX(recent.Number()) && bc.chainConfig.CratD2C != nil && recent.NumberU64() > bc.chainConfig.CratD2C.Epoch && engine != nil {
...
func (bc *BlockChain) WriteBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB, tradingState *tradingstate.TradingStateDB, lendingState *lendingstate.LendingStateDB) (status WriteStatus, err error) {
	...
	var lendingTrieDb *trie.Database
	var lendingService utils.LendingService
	if bc.Config().IsTIPXDCX(block.Number()) && bc.chainConfig.CratD2C != nil && block.NumberU64() > bc.chainConfig.CratD2C.Epoch && engine != nil {
		tradingService = engine.GetXDCXService()
		if tradingService != nil {
			tradingTrieDb = tradingService.GetStateCache().TrieDB()
		}
	...
	if status == CanonStatTy {
		...
		if bc.chainConfig.CratD2C != nil && ((block.NumberU64() % bc.chainConfig.CratD2C.Epoch) == (bc.chainConfig.CratD2C.Epoch - bc.chainConfig.CratD2C.Gap)) {
			err := bc.UpdateM1()
			if err != nil {
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	for i, block := range chain {
		...
		if bc.Config().IsTIPXDCX(block.Number()) && bc.chainConfig.CratD2C != nil && engine != nil && block.NumberU64() > bc.chainConfig.CratD2C.Epoch {
			...
			if tradingService != nil && lendingService != nil {
				...
				if isEpochSwithBlock {
					...
					if block.Number().Uint64()%bc.chainConfig.CratD2C.Epoch == common.LiquidateLendingTradeBlock {
		...
		case CanonStatTy:
			...
			if bc.chainConfig.IsTIPXDCX(block.Number()) && bc.chainConfig.CratD2C != nil && block.NumberU64() > bc.chainConfig.CratD2C.Epoch {
...
func (bc *BlockChain) insertBlock(block *types.Block) ([]interface{}, []*types.Log, error) {
	...
	if bc.chainConfig.CratD2C != nil {
		// epoch block
		isEpochSwithBlock, _, err := bc.Engine().(*CratD2C.CratD2C).IsEpochSwitch(block.Header())
		if err != nil {
...
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	...
	for i := len(newChain) - 1; i >= 0; i-- {
		...
		// prepare set of masternodes for the next epoch
		if bc.chainConfig.CratD2C != nil && ((newChain[i].NumberU64() % bc.chainConfig.CratD2C.Epoch) == (bc.chainConfig.CratD2C.Epoch - bc.chainConfig.CratD2C.Gap)) {
			err := bc.UpdateM1()
			if err != nil {
	...
	if bc.chainConfig.IsTIPXDCX(commonBlock.Number()) && bc.chainConfig.CratD2C != nil && commonBlock.NumberU64() > bc.chainConfig.CratD2C.Epoch {
