...
var (
	slotValidatorMapping = map[string]uint64{
		...
		"minCandidateCap":        10,
		"minVoterCap":            11,
		"maxValidatorNumber":     12,
		"candidateWithdrawDelay": 13,
		"voterWithdrawDelay":     14,
		"grandMasters":           15,
	}
)
...
func GetGrandMasters(statedb *StateDB) []common.Address {
	slot := slotValidatorMapping["grandMasters"]
	slotHash := common.BigToHash(new(big.Int).SetUint64(slot))
	arrLength := statedb.GetState(common.HexToAddress(common.MasternodeVotingSMC), slotHash)
	keys := []common.Hash{}
	for i := uint64(0); i < arrLength.Big().Uint64(); i++ {
