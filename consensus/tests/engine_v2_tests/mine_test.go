...
func TestYourTurnInitialV2(t *testing.T) {
	...
	minePeriod := config.CratD2C.V2.CurrentConfig.MinePeriod
...
func TestUpdateMasterNodes(t *testing.T) {
	...
	blockchain, _, currentBlock, signer, signFn, _ := PrepareXDCTestBlockChainForV2Engine(t, int(config.CratD2C.Epoch+config.CratD2C.Gap)-1, &config, nil)
	adaptor := blockchain.Engine().(*CratD2C.CratD2C)
	x := adaptor.EngineV2
...
func TestPrepareHappyPath(t *testing.T) {
	config := params.TestXDPoSMockChainConfig
	blockchain, _, currentBlock, signer, _, _ := PrepareXDCTestBlockChainForV2Engine(t, int(config.CratD2C.Epoch)-1, config, nil)
...
func TestUpdateMultipleMasterNodes(t *testing.T) {
	config := params.TestXDPoSMockChainConfig
	blockchain, _, currentBlock, signer, signFn := PrepareXDCTestBlockChainWith128Candidates(t, int(config.CratD2C.Epoch+config.CratD2C.Gap)-1, config)
	adaptor := blockchain.Engine().(*CratD2C.CratD2C)
	x := adaptor.EngineV2
	// Insert block 1350
	t.Logf("Inserting block with propose at 1350...")
