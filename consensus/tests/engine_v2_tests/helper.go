...
import (
	...
	"github.com/CratD2C-SmartChain/cratd2cchain/contracts"
...
func getCommonBackend(t *testing.T, chainConfig *params.ChainConfig, signer common.Address) *backends.SimulatedBackend {
	...
	// Skip block verification, this is just for create sm address
	engine := contractBackendForSC.GetBlockChain().Engine().(*CratD2C.CratD2C)
	engine.EngineV2.UpdateSkipV2Validation(true)
	contractBackendForSC.Commit() // Write into database(state)
	engine.EngineV2.UpdateSkipV2Validation(false)
...
func GetSnapshotSigner(bc *BlockChain, header *types.Header) (signersList, error) {
	engine := bc.Engine().(*CratD2C.CratD2C)
...
func PrepareXDCTestBlockChainForV2Engine(t *testing.T, numOfBlocks int, chainConfig *params.ChainConfig, forkedBlockOptions *ForkedBlockOptions) (*BlockChain, *backends.SimulatedBackend, *types.Block, common.Address, func(account accounts.Account, hash []byte) ([]byte, error), *types.Block) {
	// Preparation
	var err error
	signer, signFn, err := backends.SimulateWalletAddressAndSignFn("./testkey.json")
	if err != nil {
	...
	engine := blockchain.Engine().(*CratD2C.CratD2C)
	...
	for i := 1; i <= numOfBlocks; i++ {
		...
		if int64(i) > chainConfig.CratD2C.V2.SwitchBlock.Int64() {
			blockCoinBase = signer.Hex()
		}
		roundNumber = int64(i) - chainConfig.CratD2C.V2.SwitchBlock.Int64()
		...
		if i%int(chainConfig.CratD2C.Gap) == 0 {
			blockchain.UpdateM1()
		}
		...
		// First v2 block
		if (int64(i) - chainConfig.CratD2C.V2.SwitchBlock.Int64()) == 1 {
			lastv1BlockNumber := block.Header().Number.Uint64() - 1
			checkpointBlockNumber := lastv1BlockNumber - lastv1BlockNumber%chainConfig.CratD2C.Epoch
	...
	engine.EngineV2.ProcessQCFaker(blockchain, extraField.QuorumCert)
	...
	engine.EngineV2.UpdateParams(currentBlock.Header()) // it will be triggered automatically on the real code by other process
...
func PrepareXDCTestBlockChainWithPenaltyForV2Engine(t *testing.T, numOfBlocks int, chainConfig *params.ChainConfig) (*BlockChain, *backends.SimulatedBackend, *types.Block, common.Address, func(account accounts.Account, hash []byte) ([]byte, error)) {
	...
	// Authorise
	blockchain.Engine().(*CratD2C.CratD2C).Authorize(signer, signFn)
...
// V2 concensus engine, pretend
func PrepareQCandProcess(t *testing.T, blockchain *BlockChain, currentBlock *types.Block) {
	engine := blockchain.Engine().(*CratD2C.CratD2C)
	var extraField types.ExtraFields_v2
	err := utils.DecodeBytesExtraFields(currentBlock.Extra(), &extraField)
	if err != nil {
	...
	QC := &types.QuorumCert{
		...
		Signatures: nil,
		GapNumber:  extraField.QuorumCert.GapNumber,
	}

	engine.EngineV2.ProcessQCFaker(blockchain, QC)
}
...
func CreateBlock(blockchain *BlockChain, chainConfig *params.ChainConfig, startingBlock *types.Block, blockNumber int, roundNumber int64, blockCoinBase string, signer common.Address, signFn func(account accounts.Account, hash []byte) ([]byte, error), penalties []common.Address, signersKey []*ecdsa.PrivateKey, merkleRoot string) *types.Block {
	...
	if big.NewInt(int64(blockNumber)).Cmp(chainConfig.CratD2C.V2.SwitchBlock) == 1 { // Build engine v2 compatible extra data field
		...
		if header.Number.Uint64()%chainConfig.CratD2C.Epoch == 0 {
			// epoch switch blocks, we need v2 validators aka masternodes
			header.Validators = masternodes
		} else if (header.Number.Uint64()%chainConfig.CratD2C.Epoch) == (chainConfig.CratD2C.Epoch-chainConfig.CratD2C.Gap+1) || header.Number.Uint64() == 1 { // This is the gapPlusOneBlock
...
func findSignerAndSignFn(bc *BlockChain, header *types.Header, signer common.Address, signFn func(account accounts.Account, hash []byte) ([]byte, error), config *params.ChainConfig) (common.Address, func(account accounts.Account, hash []byte) ([]byte, error)) {
	...
	if header.Number.Cmp(config.CratD2C.V2.SwitchBlock) > 0 {
		...
		index := uint64(round) % config.CratD2C.Epoch % uint64(len(masterNodes))
...
func sealHeader(bc *BlockChain, header *types.Header, signer common.Address, signFn func(account accounts.Account, hash []byte) ([]byte, error)) {
	// Sign all the things and seal it
	signedBlockHeader := bc.Engine().(*CratD2C.CratD2C).SigHash(header)
...
func generateV2Extra(roundNumber int64, currentBlock *types.Block, signer common.Address, signFn func(account accounts.Account, hash []byte) ([]byte, error), accKeys []*ecdsa.PrivateKey) []byte {
	...
	gapNumber := uint64(0)
	if currentBlock.NumberU64() >= 900 {
		gapNumber = currentBlock.Number().Uint64() - currentBlock.Number().Uint64()%params.TestXDPoSMockChainConfig.CratD2C.Epoch - params.TestXDPoSMockChainConfig.CratD2C.Gap
	}
