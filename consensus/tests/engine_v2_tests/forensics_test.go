...
func TestSetCommittedQCsInOrder(t *testing.T) {
	...
	forensics := blockchain.Engine().(*CratD2C.CratD2C).EngineV2.GetForensicsFaker()
...
func TestForensicsMonitoring(t *testing.T) {
	...
	err = forensics.ForensicsMonitoring(blockchain, blockchain.Engine().(*CratD2C.CratD2C).EngineV2, newIncomingQcHeaders, *incomingQC)
	assert.Nil(t, err)
}
...
func TestForensicsMonitoringNotOnSameChainButHaveSameRoundQC(t *testing.T) {
	...
	*forkRoundDifference = 1
	...
	err = forensics.ForensicsMonitoring(blockchain, blockchain.Engine().(*CratD2C.CratD2C).EngineV2, forkedHeaders, *incomingQC)
	assert.Nil(t, err)
...
func TestVoteEquivocationDifferentRound(t *testing.T) {
	...
	*forkRoundDifference = 1
	...
	err = forensics.ProcessVoteEquivocation(blockchain, blockchain.Engine().(*CratD2C.CratD2C).EngineV2, incomingVote)
	assert.Nil(t, err)
	// Check SendForensicProof triggered
	for {
		select {
