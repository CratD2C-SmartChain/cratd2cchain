...
func loadSnapshot(db ethdb.Database, hash common.Hash) (*SnapshotV2, error) {
	blob, err := db.Get(append([]byte("CratD2C-V2-"), hash[:]...))
	if err != nil {
		return nil, err
	}
	snap := new(SnapshotV2)
...
func storeSnapshot(s *SnapshotV2, db ethdb.Database) error {
	...
	return db.Put(append([]byte("CratD2C-V2-"), s.Hash[:]...), blob)
}
...
func (x *CratD2C_v2) getSnapshot(chain consensus.ChainReader, number uint64, isGapNumber bool) (*SnapshotV2, error) {
...
// snapshot retrieves the authorization snapshot at a given point in time.
func (x *CratD2C_v2) getSnapshotByHash(gapBlockHash common.Hash) (*SnapshotV2, error) {
	// If an in-memory SnapshotV2 was found, use that
	if s, ok := x.snapshots.Get(gapBlockHash); ok {
		snap := s.(*SnapshotV2)
