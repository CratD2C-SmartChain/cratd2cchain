...
func SubRelayerFee(relayer common.Address, fee *big.Int, statedb *state.StateDB) error {
	...
	if balance.Cmp(fee) < 0 {
		return errors.Errorf("relayer %s isn't enough CRAT fee", relayer.String())
	} else {
...
func CheckRelayerFee(relayer common.Address, fee *big.Int, statedb *state.StateDB) error {
	...
	if new(big.Int).Sub(balance, fee).Cmp(new(big.Int).Mul(common.BasePrice, common.RelayerLockedFund)) < 0 {
		return errors.Errorf("relayer %s isn't enough CRAT fee : balance %d , fee : %d ", relayer.Hex(), balance.Uint64(), fee.Uint64())
	}
	return nil
}
func AddTokenBalance(addr common.Address, value *big.Int, token common.Address, statedb *state.StateDB) error {
	...
	if token.String() == common.XDCNativeAddress {
		...
		log.Debug("ApplyXDCXMatchedTransaction settle balance: ADD TOKEN CRAT NATIVE BEFORE", "token", token.String(), "address", addr.String(), "balance", balance, "orderValue", value)
		statedb.AddBalance(addr, value)
		balance = statedb.GetBalance(addr)
		log.Debug("ApplyXDCXMatchedTransaction settle balance: ADD CRAT NATIVE BALANCE AFTER", "token", token.String(), "address", addr.String(), "balance", balance, "orderValue", value)
...
func SubTokenBalance(addr common.Address, value *big.Int, token common.Address, statedb *state.StateDB) error {
	...
	if token.String() == common.XDCNativeAddress {
		balance := statedb.GetBalance(addr)
		log.Debug("ApplyXDCXMatchedTransaction settle balance: SUB CRAT NATIVE BALANCE BEFORE", "token", token.String(), "address", addr.String(), "balance", balance, "orderValue", value)
		...
		balance = statedb.GetBalance(addr)
		log.Debug("ApplyXDCXMatchedTransaction settle balance: SUB CRAT NATIVE BALANCE AFTER", "token", token.String(), "address", addr.String(), "balance", balance, "orderValue", value)
...
func CheckSubTokenBalance(addr common.Address, value *big.Int, token common.Address, statedb *state.StateDB, mapBalances map[common.Address]map[common.Address]*big.Int) (*big.Int, error) {
	...
	if token.String() == common.XDCNativeAddress {
		...
		log.Debug("CheckSubTokenBalance settle balance: SUB CRAT NATIVE BALANCE ", "token", token.String(), "address", addr.String(), "balance", balance, "value", value, "newBalance", newBalance)
...
func CheckAddTokenBalance(addr common.Address, value *big.Int, token common.Address, statedb *state.StateDB, mapBalances map[common.Address]map[common.Address]*big.Int) (*big.Int, error) {
	...
	if token.String() == common.XDCNativeAddress {
		...
		log.Debug("CheckAddTokenBalance settle balance: ADD CRAT NATIVE BALANCE ", "token", token.String(), "address", addr.String(), "balance", balance, "value", value, "newBalance", newBalance)
		return newBalance, nil
	}
	// TRC tokens
	if statedb.Exist(token) {
...
func CheckSubRelayerFee(relayer common.Address, fee *big.Int, statedb *state.StateDB, mapBalances map[common.Address]*big.Int) (*big.Int, error) {
	...
	if balance.Cmp(fee) < 0 {
		return nil, errors.Errorf("relayer %s isn't enough CRAT fee", relayer.String())
	} else {
