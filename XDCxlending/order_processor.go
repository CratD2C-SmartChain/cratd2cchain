...
func (l *Lending) ProcessCancelOrder(header *types.Header, lendingStateDB *lendingstate.LendingStateDB, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, chain consensus.ChainContext, coinbase common.Address, lendingOrderBook common.Hash, order *lendingstate.LendingItem) (error, bool) {
	...
	// relayers pay CRAT for masternode
...
// - Have pairs with CRAT:
// -  lendToken/CRAT and CollateralToken/CRAT
// -  CRAT/lendToken and CRAT/CollateralToken
func (l *Lending) GetCollateralPrices(header *types.Header, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, collateralToken common.Address, lendingToken common.Address) (*big.Int, *big.Int, error) {
	// lendTokenXDCPrice: price of ticker lendToken/CRAT
	// collateralXDCPrice: price of ticker collateralToken/CRAT
	...
	collateralPriceUpdatedFromContract := updatedBlock.Uint64()/chain.Config().CratD2C.Epoch == header.Number.Uint64()/chain.Config().CratD2C.Epoch
	...
	inverseCollateralPriceUpdatedFromContract := updatedBlock.Uint64()/chain.Config().CratD2C.Epoch == header.Number.Uint64()/chain.Config().CratD2C.Epoch
	...
	// Calculate collateral/LendToken price from collateral/CRAT, lendToken/CRAT
	...
	log.Debug("GetCollateralPrices: Calculate collateral/LendToken price from collateral/CRAT, lendToken/CRAT", "collateralPrice", collateralPrice,
...
func (l *Lending) GetXDCBasePrices(header *types.Header, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, token common.Address) (*big.Int, error) {
	...
	tokenXDCPriceUpdatedFromContract := updatedBlock.Uint64()/chain.Config().CratD2C.Epoch == header.Number.Uint64()/chain.Config().CratD2C.Epoch
	...
	if token == common.HexToAddress(common.XDCNativeAddress) {
		...
		// otherwise, getting from XDCx lendToken/CRAT
		log.Debug("Getting token/CRAT price from contract", "price", tokenXDCPriceFromContract)
		...
		XDCTokenPriceUpdatedFromContract := updatedBlock.Uint64()/chain.Config().CratD2C.Epoch == header.Number.Uint64()/chain.Config().CratD2C.Epoch
		if XDCTokenPriceUpdatedFromContract && XDCTokenPriceFromContract != nil && XDCTokenPriceFromContract.Sign() > 0 {
			// getting lendToken price from contract first
			// otherwise, getting from XDCx lendToken/CRAT
			log.Debug("Getting CRAT/token from contract", "price", XDCTokenPriceFromContract)
		...
		if tokenXDCPrice != nil && tokenXDCPrice.Sign() > 0 {
			log.Debug("Getting token/CRAT from XDCx", "price", tokenXDCPrice, "err", err)
			return tokenXDCPrice, nil
		}
	}
